...
- **pnpm 发布前自动化**：根 `package.json` 新增 `release:sync-version` 与 `release:check-version`，分别用于同步 root/CLI 版本与校验 tag/版本/CLI scope 一致性。建议发布前先执行 `pnpm release:sync-version <version>` 和 `pnpm release:check-version <version>`。
